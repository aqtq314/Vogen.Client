...
type VoiceLibMetadata = {
    ...
    [<JsonProperty("builtBy", Required=Required.DisallowNull)>] BuiltBy : string
    [<JsonProperty("voiceBy", Required=Required.DisallowNull)>] VoiceBy : string }

let voiceLibs =
    Directory.GetFiles(appDir, "voicelib.*.vogeon")
